...
public class AdminCategoryController {
	...
	void initialize() throws ClassNotFoundException, SQLException, InstantiationException, IllegalAccessException {
		assert tb_category != null
				: "fx:id=\"tb_category\" was not injected: check your FXML file 'Admin_category.fxml'.";
		...
		assert tf_date_created != null
				: "fx:id=\"tf_date_created\" was not injected: check your FXML file 'Admin_category.fxml'.";
		...
		tb_category.getColumns().addAll(col_item_id, col_item_name, col_item_date_created);
			...
			removeItem.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent event) {
					...
					if (alert.getResult() == ButtonType.YES) {
						...
						// get count
						int cateCount = 0;
						String getCateUsedCount = "SELECT COUNT(*) FROM productitems, productcategory WHERE productcategory.id = productitems.categoryid AND productcategory.id = '"
								+ ca.getId() + "'";
						try {
							new DBInitialize().DBInitialize();
							new DBInitialize();
						...
						if (cateCount > 0) {
							...
							Alert aal = new Alert(AlertType.ERROR,
									"Cannot Delete! This Category has been used in product items. You can delete the product items that link with this category and try again. Thanks!");
							aal.showAndWait();

						} else {
							// do stuff
							String removequery = "DELETE FROM `productcategory` WHERE  productcategory.id = '"
									+ ca.getId() + "';";
							try {
								new DBInitialize().DBInitialize();
								new DBInitialize();
	...
	void onAddAction(ActionEvent event) throws SQLException {
		...
		if (tf_id.getText().equals("") || tf_name.getText().equals("") || tf_date_created.getText().equals("")) {
			...
			try {
				String addQuery = "INSERT INTO `productcategory`(`id`, `name`, `datecreated`) VALUES ("
						+ Integer.parseInt(id) + ", '" + name + "', '" + datecreated + "')";
				new DBInitialize();
	...
	void onUpdateAction(ActionEvent event) throws SQLException {
		if (tf_id.getText().equals("") || tf_name.getText().equals("") || tf_date_created.getText().equals("")) {
			...
			String updateQuery = "UPDATE `productcategory` SET `name`='" + name + "',`datecreated`='" + datecreated
					+ "' WHERE `id`=" + id + ";";
			new DBInitialize();
